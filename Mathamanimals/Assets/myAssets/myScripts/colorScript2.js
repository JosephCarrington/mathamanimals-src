...
//	yellow = Globals.AccentColor;
	...
	if (!cycleColors && !rainBow){
			...
			if (setBlack){renderer.material.color = black;}
			if (setBlue){renderer.material.color = blue;}
			if (setBrown){renderer.material.color = brown;}
			if (setGray){renderer.material.color = gray;}
			if (setGreen){renderer.material.color = green;}
			if (setOrange){renderer.material.color = orange;}
			if (setPink){renderer.material.color = pink;}
			if (setPurple){renderer.material.color = purple;}
			if (setRed){renderer.material.color = red;}
			if (setYellow){renderer.material.color = yellow;}
		}else{renderer.material.color = myColor;}
		...
		if (cycleColors){
			...
			if (countUp > 1){
				...
				if (randomPick == 1){renderer.material.color = pink;}
				if (randomPick == 2){renderer.material.color = blue;}
				if (randomPick == 3){renderer.material.color = yellow;}
